...
test_df_CD$VALUE[which(is.na(test_df_CD$measure_transformed))]
...
#Look at distribution:
 p1 <-  ggplot(test_df_CD)+
    geom_histogram(aes(x=as.numeric(VALUE)), bins=30) + theme_classic()+ylab("Count") + xlab("Vitamin D2+D3")
 p2 <-  ggplot(test_df_CD)+
    geom_histogram(aes(x=log(as.numeric(VALUE))+1), bins=30)+ theme_classic()+ylab("Count") + xlab("Vitamin D2+D3 - log transformed")
...
#Run model and extract results - UC
  ...
  f1 <- lmer(measure_transformed~  + C_KON + age + IBD * time2 + (1|lbnr)+(1|LABORATORIUM_IDCODE), data = test_df_UC, control=lmerControl(optimizer = "bobyqa"))
  summary(f1)
...
test_df_CD <- test_df_CD%>% mutate(time_interval = ifelse(sampling_to_diag<365.24, 1, ifelse(sampling_to_diag<2*365.25, 2, ifelse(sampling_to_diag<3*365.25,3, ifelse(sampling_to_diag<4*365.25,4, ifelse(sampling_to_diag<5*365.25,5, ifelse(sampling_to_diag<6*365.25,6,ifelse(sampling_to_diag<7*365.25,7, 8))))))))
...
res_CD
write.table(res_CD, "F:/Projekter/FSEID00001565/Data/Subprojects/Marie_IBD_clinical_biomarker/results/VitD_restable_CD_onlyGP.txt", quote=F, row.names=F)

p <- ggplot(data=test_df_CD)+
    geom_boxplot(aes(x=as.factor(time_interval), y=measure_transformed, color=IBD))+
    ggtitle("Crohn's Disease: Vitamin D2 and D3")+theme_classic()+xlab("Years before diagnosis")+ylab("Measure (sqrt transformed)")


# UC:
# Add time interval column
max(test_df_UC$sampling_to_diag)/365.25
test_df_UC <- test_df_UC %>% mutate(time_interval = ifelse(sampling_to_diag<365.24, 1, ifelse(sampling_to_diag<2*365.25, 2, ifelse(sampling_to_diag<3*365.25,3, ifelse(sampling_to_diag<4*365.25,4, ifelse(sampling_to_diag<5*365.25,5, ifelse(sampling_to_diag<6*365.25,6,ifelse(sampling_to_diag<7*365.25,7,ifelse(sampling_to_diag<8*365.25,8, ifelse(sampling_to_diag<9*365.25,9,ifelse(sampling_to_diag<10*365.25, 10, 11)))))))))))
...
res_UC
write.table(res_UC, "F:/Projekter/FSEID00001565/Data/Subprojects/Marie_IBD_clinical_biomarker/results/VitD_restable_UC_onlyGP.txt", quote=F, row.names=F)

q <- ggplot(data=test_df_UC)+
    geom_boxplot(aes(x=as.factor(time_interval), y=measure_transformed, color=IBD))+
    ggtitle("Ulcerative colitis: Vitamin D2 and D3")+theme_classic()+xlab("Years before diagnosis")+ylab("Measure (sqrt transformed)")
...
colnames(res_CD)[1:5] <- c("Estimate", "SE", "df", "t_value", "p_value")
res_CD
write.table(res_CD, "F:/Projekter/FSEID00001565/Data/Subprojects/Marie_IBD_clinical_biomarker/results/One_VitD_restable_CD_onlyGP.txt", quote=F, row.names=F)
...
colnames(res_UC)[1:5] <- c("Estimate", "SE", "df", "t_value", "p_value")
res_UC
write.table(res_UC, "F:/Projekter/FSEID00001565/Data/Subprojects/Marie_IBD_clinical_biomarker/results/One_VitD_restable_UC_onlyGP.txt", quote=F, row.names=F)
```
...
res
write.table(res, "F:/Projekter/FSEID00001565/Data/Subprojects/Marie_IBD_clinical_biomarker/results/VitD_UC_vs_CD_onlyGP.txt", quote=F, row.names=F)
```

## Sensitivity analysis: 2 IBD diagnoses
```{r}
test_df_CD <- read.csv("F:/Projekter/FSEID00001565/Data/Subprojects/Marie_IBD_clinical_biomarker/Sensitivity_Vitamin D1_CD_onlyGP.txt", sep=" ")
test_df_UC <- read.csv("F:/Projekter/FSEID00001565/Data/Subprojects/Marie_IBD_clinical_biomarker/Sensitivity_Vitamin D1_UC_onlyGP.txt", sep=" ")
...
sub_UC <- test_df_UC %>% filter(as.numeric(VALUE)>375)
m_UC <- median(as.numeric(sub_UC$VALUE))
test_df_UC$VALUE <- gsub(">375", m_UC, test_df_UC$VALUE)
rm(sub_UC)
...
#Look at distribution:
  ...
  ggplot(test_df_CD)+
    geom_histogram(aes(x=sqrt(as.numeric(VALUE))), bins=30)
...
# Add time interval column
max(test_df_CD$sampling_to_diag)/365.25
test_df_CD <- test_df_CD%>% mutate(time_interval = ifelse(sampling_to_diag<365.24, 1, ifelse(sampling_to_diag<2*365.25, 2, ifelse(sampling_to_diag<3*365.25,3, ifelse(sampling_to_diag<4*365.25,4, ifelse(sampling_to_diag<5*365.25,5, ifelse(sampling_to_diag<6*365.25,6,ifelse(sampling_to_diag<7*365.25,7,ifelse(sampling_to_diag<8*365.25,8, ifelse(sampling_to_diag<9*365.25,9,ifelse(sampling_to_diag<10*365.25, 10, 11)))))))))))
...
colnames(res_CD)[1:5] <- c("Estimate", "SE", "df", "t_value", "p_value")
res_CD
write.table(res_CD, "F:/Projekter/FSEID00001565/Data/Subprojects/Marie_IBD_clinical_biomarker/results/Sensitivity_VitD_restable_CD_onlyGP.txt", quote=F, row.names=F)
...
colnames(res_UC)[1:5] <- c("Estimate", "SE", "df", "t_value", "p_value")
res_UC
write.table(res_UC, "F:/Projekter/FSEID00001565/Data/Subprojects/Marie_IBD_clinical_biomarker/results/Sensitivity_VitD_restable_UC_onlyGP.txt", quote=F, row.names=F)

```

## Comorb sensitivity analysis:
```{r}
test_df_CD <- read.csv("F:/Projekter/FSEID00001565/Data/Subprojects/Marie_IBD_clinical_biomarker/Vitamin D1_CD_comorb_onlyGP.txt", sep=" ")
test_df_UC <- read.csv("F:/Projekter/FSEID00001565/Data/Subprojects/Marie_IBD_clinical_biomarker/Vitamin D1_UC_comorb_onlyGP.txt", sep=" ")
...
colnames(res_CD)[1:5] <- c("Estimate", "SE", "df", "t_value", "p_value")
res_CD
write.table(res_CD, "F:/Projekter/FSEID00001565/Data/Subprojects/Marie_IBD_clinical_biomarker/results/VitD_restable_CD_comorb_onlyGP.txt", quote=F, row.names=F)
...
colnames(res_UC)[1:5] <- c("Estimate", "SE", "df", "t_value", "p_value")
res_UC
write.table(res_UC, "F:/Projekter/FSEID00001565/Data/Subprojects/Marie_IBD_clinical_biomarker/results/VitD_restable_UC_comorb_onlyGP.txt", quote=F, row.names=F)
